...
public class VerticalDoubleSplitter implements IFractalSplitter{
	...
	public void draw(FractalPart pPart, Canvas pCanvas) {
		...
		pCanvas.drawLine(pPart.getField().left + pPart.getWidth() / 2, pPart.getField().top, 
				pPart.getField().left + pPart.getWidth() / 2, pPart.getField().bottom, paint);		
	}
	...
	public HashSet<FractalPart> split(FractalPart pPart) {
		...
		result.add(new FractalPart(new RectF(pPart.getField().left, pPart.getField().top, 
				pPart.getField().left + pPart.getWidth() / 2, pPart.getField().bottom)));
		
		result.add(new FractalPart(new RectF(pPart.getField().left + pPart.getWidth() / 2, pPart.getField().top, 
