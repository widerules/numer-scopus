...
public class ColorSpriteOrnamentManager extends AssetOrnamentManager {
	
	Random mRnd;
	
	public ColorSpriteOrnamentManager(final Context pContext, final String pAssetBasePath,
			final String pFileNamePrefix, final TextureManager pTextureManager,
			final int pTextureSize){
		super(pContext, pAssetBasePath, pFileNamePrefix, pTextureManager, pTextureSize);
	...
	public IEntity getEntity(int pOrnamentId, RectF pField, byte pFillMethod) {
		IEntity entity = super.getEntity(mRnd.nextInt(super.getOrnamentCount()), pField, pFillMethod);
